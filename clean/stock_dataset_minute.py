...
start_date = "20040101"
end_date = "20231231"
conn = sqlite3.connect(r"D:\redhand\clean\data\tushare_db\stock_minute.db",check_same_thread=False)
...
def compute_data_y(df,fh,df_index,is_merge_index):
    df = indicatior(df)
    if is_merge_index:
        df = df.merge(df_index, how="left", suffixes=["_stock", "_index"], on="trade_date")
        df = df.drop(labels=["ts_code", "trade_date", "ts_code_sh_index", "ts_code_sz_index","trade_time","adj"], axis=1)
    else:
    ...
    # 将涨幅分类为0到8，进行9分类
    df_data = df.copy()
    df["target"] = df["next_n_pct_chg"].apply(categorize_minute)
    # 进行数据归一化
...
class StockPytorchDatasetNew(Dataset):
    ...
    def get_length(self):
        ...
        df = pd.read_sql("select ts_code from stock_all",conn)
        for key, ts_code in enumerate(np.unique(df["ts_code"])):
            print(ts_code)
            df = get_one_stock_data_from_sqlite(ts_code, start_date, end_date, False)
            if len(df) < self.step_length + self.fh + 1:
                continue
            df_data = compute_data_y(df,self.fh,self.df_index,False)
...
def new_fit(step_length,fh,vars=96):
    import keras
    ...
    # 模型初始化，
    model_ = network.build_model((step_length, vars), 9)
    model_save_path = r"D:\redhand\clean\data\state_dict\inceptiontime_minute_{}_{}.keras".format(step_length,fh)
    # 模型参数初始化
    # model_ = keras.saving.load_model(model_save_path)
    # 开始训练
    csv_logger = keras.callbacks.CSVLogger(r"D:\redhand\clean\data\log\inceptiontime_minute_log_{}_{}.csv".format(step_length,fh), separator=",", append=True)
